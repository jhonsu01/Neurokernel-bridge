...
from ..models import (
    ...
    Decision, DecisionTier, DecisionResult, EventDimension,
)
from ..config import DecisionConfig
from ..memory.embeddings import NGramEmbeddingFunction


class CacheEngine:
    """Tier 2: SQLite + n-gram vector similarity cache. ~1-5ms latency.

    Replaces ChromaDB to avoid onnxruntime/Rust AVX requirements.
    Uses n-gram embeddings with cosine similarity for pattern matching.
    """
    ...
    def lookup(self, event: BaseEvent) -> Optional[DecisionResult]:
        ...
        if best_sim < self.threshold or best_row is None:
            return None
